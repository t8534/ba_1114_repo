...
void SPITESTS_Init(SSP_Dev_t *dev)
{

	dev->Device = LPC_SSP1;
	dev->FrameFormat = SSP_FRAME_SPI;
	dev->DataSize = SSP_DATABITS_8;
	dev->CPOL = SSP_SPI_CPOL_LO;
	dev->CPHA = SSP_SPI_CPHA_FIRST;
	dev->LoopBackMode = SSP_LOOPBACK_ON;
	dev->Mode = SSP_MASTER_MODE;

	dev->SCR = 0x7;              /* CR0->SerialClockRate */
	dev->CPSDVSR = 0x02;         /* SSPxCPSR->CPSDVSR */
	dev->DIV = 0x02;             /* SSPxCLKDIV->DIV */

	dev->SlaveOutputDisable = SSP_SLAVE_OUTPUT_ENABLE;
	dev->transferType = SSP_TRANSFER_POLLING;
	dev->InterruptCondition = SSP_ISR_NOFLAG_SET;
	dev->ISR_Processing = NULL;
	dev->SSEL_Mode = SSP_SSEL_AUTO;
	dev->IO_pins.MOSI_pin = SSP_NO_PIN;
	dev->IO_pins.MISO_pin = SSP_NO_PIN;
	dev->IO_pins.SCK_pin = SSP_NO_PIN;
	dev->IO_pins.SSEL_pin = SSP_NO_PIN;
